...
parser.add_argument("--activate_dp", type=int, default=False, help="Activate differential privacy")
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(dataset_train_object.featureSize, self.disDim)
        self.fc2 = nn.Linear(self.disDim, int(self.disDim))
        self.fc3 = nn.Linear(self.disDim, int(self.disDim))
        self.fc4 = nn.Linear(int(self.disDim), 1)
    ...
    for epoch in range(opt.n_epochs):
        ...
        for i, samples in enumerate(dataloader_train):
            ...
            while j < n_iter_D:
                ...
                errD_real = torch.mean(discriminatorModel(real_samples),axis=0)
                ...
                # Generate a batch of images
                fake_samples = generatorModel(z).detach()
                fake_samples = autoencoderDecoder(fake_samples)

                errD_fake = torch.mean(discriminatorModel(fake_samples.detach()),axis=0)
            ...
            # Loss measures generator's ability to fool the discriminator
            errG = -torch.mean(discriminatorModel(fake_samples),axis=0)
            errG.backward(one)
